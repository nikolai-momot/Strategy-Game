...
public class Player_AI : Player{
	...
	public void MoveArmies(){        
        ...
        foreach (Army a in Armies) {
            ...
            if ( 50+Defencivness > RecDes) {
                Army HTVArmy = getHTVArmy(a);
                StratObj HTVStratObj = getHTVStratObj();
                if (HTVArmy.getStrategicValueForAI(a) > HTVStratObj.getStrategicValueForAI(HQ.getMapPosition())) {
                    a.AttackTarget(HTVArmy);
                } else {
                    a.AttackTarget(HTVStratObj);
                }
            }else{
    ...
    public StratObj getHTVStratObj() {
        ...
        for (int i = 0; i < GameManager.Locations.Count; i++) {
            if (GameManager.Locations[i].OwnerID != this.ID) {
                if (GameManager.Locations[hvt_index].getStrategicValueForAI(HQ.getMapPosition()) < GameManager.Locations[i].getStrategicValueForAI(HQ.getMapPosition())) {
                    hvt_index = i;
                }
            }
        }
