	...
	public void AutoResolve() {
		if (defender != null) { //Is an Army defending
			...
			if (atk > def) { //attacker wins
				attacker.AddWin();
				attacker.TakeLosses(Random.Range(0, (atk - def) / 3));
			} else { //defender wins
				defender.AddLoss();
				defender.TakeLosses(Random.Range((def - atk) / 10, (def - atk)));
				defender.Retreat();
			}
		} else { //Is an Objective Defending
			if (attacker.OwnerID == Objective.OwnerID) { return; } //Someone else took it first...
			...
			if (atk > def) { //attacker wins
				...
				Objective.TakeLosses(Random.Range((def - atk) / 10, (def - atk)));
